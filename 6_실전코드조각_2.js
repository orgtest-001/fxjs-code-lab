...
var total_quantity = _.reduce(function (tq, product) {
    return tq + _.reduce(product.sizes, function(tq) {
        return tq + tq.quantity;
    }, 0);
}, 0)
...
_.go(
    products,
    _filter(_get('is_selected')),
    _reduce(function (tp, product) {
        return _.reduce(product.size, function(tp, size) {
            return tp + (size.price + product.price) * size.quantity;
        }, tp)
    }, 0),
    console.log
)